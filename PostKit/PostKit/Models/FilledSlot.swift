...
enum SlotFiller {
    ...
    static func fillOne(
        ...
        if let picked = available.randomElement() {
            ...
            // Prefer the slot's requested pillar over photo.pillarID (singular) — a photo can
            // belong to multiple pillars and pillarID may point to a different one than requested.
            filled.activePillarID = slot.pillarIDs.first { picked.pillarIDs.contains($0) || picked.pillarID == $0 }
                ?? picked.pillarID
            ...
            filled.activePillarID = slot.pillarIDs.first { fallback.pillarIDs.contains($0) || fallback.pillarID == $0 }
                ?? fallback.pillarID
...
struct PostEditorFeature {
    ...
    enum Action: BindableAction {
        ...
        case toggleCaptionSection
        case toggleScheduleSection
        case addSlotTapped
        case closeTapped
    ...
    var body: some ReducerOf<Self> {
        ...
        Reduce { state, action in
            ...
            case .toggleCaptionSection:
                state.isCaptionExpanded.toggle()
                return .none

            case .toggleScheduleSection:
                state.isScheduleExpanded.toggle()
                return .none

            case .addSlotTapped:
                let newSlot = TemplateSlotData(
                    id: UUID(),
                    name: "Slot \(state.template.slots.count + 1)",
                    cadrages: [],
                    pillarIDs: [],
                    locations: [],
                    about: "",
                ...
                state.template.slots.append(newSlot)
                state.filledSlots.append(FilledSlot(slotData: newSlot, photoIDs: []))
                return .none
